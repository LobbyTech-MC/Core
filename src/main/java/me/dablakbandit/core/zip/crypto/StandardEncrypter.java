...
public class StandardEncrypter implements IEncrypter{
	...
	public StandardEncrypter(char[] password, int crc) throws ZipException{
		if(password == null || password.length <= 0){ throw new ZipException("input password is null or empty in standard encrpyter constructor"); }
	...
	private void init(char[] password, int crc) throws ZipException{
		if(password == null || password.length <= 0){ throw new ZipException("input password is null or empty, cannot initialize standard encrypter"); }
		...
		if(headerBytes.length < InternalZipConstants.STD_DEC_HDR_SIZE){ throw new ZipException("invalid header bytes generated, cannot perform standard encryption"); }
	...
	public int encryptData(byte[] buff) throws ZipException{
		if(buff == null){ throw new NullPointerException(); }
	...
	public int encryptData(byte[] buff, int start, int len) throws ZipException{
		...
		if(len < 0){ throw new ZipException("invalid length specified to decrpyt data"); }
	...
	protected byte[] generateRandomBytes(int size) throws ZipException{
		...
		if(size <= 0){ throw new ZipException("size is either 0 or less than 0, cannot generate header for standard encryptor"); }
