...
import me.dablakbandit.core.zip.crypto.PBKDF2.MacBasedPRF;
import me.dablakbandit.core.zip.crypto.PBKDF2.PBKDF2Engine;
import me.dablakbandit.core.zip.crypto.PBKDF2.PBKDF2Parameters;
import me.dablakbandit.core.zip.crypto.engine.AESEngine;
...
public class AESDecrypter implements IDecrypter{
	...
	public AESDecrypter(LocalFileHeader localFileHeader, byte[] salt, byte[] passwordVerifier) throws ZipException{
		...
		if(localFileHeader == null){ throw new ZipException("one of the input parameters is null in AESDecryptor Constructor"); }
	...
	private void init(byte[] salt, byte[] passwordVerifier) throws ZipException{
		if(localFileHeader == null){ throw new ZipException("invalid file header in init method of AESDecryptor"); }

		AESExtraDataRecord aesExtraDataRecord = localFileHeader.getAesExtraDataRecord();
		if(aesExtraDataRecord == null){ throw new ZipException("invalid aes extra data record - in init method of AESDecryptor"); }
		...
		if(localFileHeader.getPassword() == null || localFileHeader.getPassword().length <= 0){ throw new ZipException("empty or null password provided for AES Decryptor"); }

		byte[] derivedKey = deriveKey(salt, localFileHeader.getPassword());
		if(derivedKey == null || derivedKey.length != KEY_LENGTH + MAC_LENGTH + PASSWORD_VERIFIER_LENGTH){ throw new ZipException("invalid derived key"); }
		...
		if(derivedPasswordVerifier == null){ throw new ZipException("invalid derived password verifier for AES"); }

		if(!Arrays.equals(passwordVerifier, derivedPasswordVerifier)){ throw new ZipException("Wrong Password for file: " + localFileHeader.getFileName(), ZipExceptionConstants.WRONG_PASSWORD); }
	...
	public int decryptData(byte[] buff, int start, int len) throws ZipException{
		...
		if(aesEngine == null){ throw new ZipException("AES not initialized properly"); }

		try{

			for(int j = start; j < start + len; j += InternalZipConstants.AES_BLOCK_SIZE){
				loopCount = j + InternalZipConstants.AES_BLOCK_SIZE <= start + len ? InternalZipConstants.AES_BLOCK_SIZE : start + len - j;
	...
	// int newlyRead = raf.read(storedMacBytes, bytesRead,
	// InternalZipConstants.AES_AUTH_LENGTH - bytesRead);
	// bytesRead += newlyRead;
	// if (bytesRead != InternalZipConstants.AES_AUTH_LENGTH) {
	// throw new
	// ZipException("invalid number of bytes read for stored MAC after starting split file");
	// }
	// } else {
