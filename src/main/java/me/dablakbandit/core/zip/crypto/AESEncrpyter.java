...
public class AESEncrpyter implements IEncrypter{
	...
	public AESEncrpyter(char[] password, int keyStrength) throws ZipException{
		if(password == null || password.length == 0){ throw new ZipException("input password is empty or null in AES encrypter constructor"); }
		if(keyStrength != Zip4jConstants.AES_STRENGTH_128 && keyStrength != Zip4jConstants.AES_STRENGTH_256){ throw new ZipException("Invalid key strength in AES encrypter constructor"); }
	...
	private void init() throws ZipException{
		...
		if(keyBytes == null || keyBytes.length != KEY_LENGTH + MAC_LENGTH + PASSWORD_VERIFIER_LENGTH){ throw new ZipException("invalid key generated, cannot decrypt file"); }
	...
	public int encryptData(byte[] buff) throws ZipException{
		...
		if(buff == null){ throw new ZipException("input bytes are null, cannot perform AES encrpytion"); }
	...
	public int encryptData(byte[] buff, int start, int len) throws ZipException{
		...
		if(finished){
			...
			// non 16 byte block should be the last block of compressed data in
			// AES encryption
	...
	private static byte[] generateSalt(int size) throws ZipException{
		...
		if(size != 8 && size != 16){ throw new ZipException("invalid salt size, cannot generate salt"); }
