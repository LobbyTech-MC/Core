...
import me.dablakbandit.core.zip.crypto.AESEncrpyter;
import me.dablakbandit.core.zip.crypto.IEncrypter;
import me.dablakbandit.core.zip.crypto.StandardEncrypter;
...
public class CipherOutputStream extends BaseOutputStream{
	...
	public void putNextEntry(File file, ZipParameters zipParameters) throws ZipException{
		if(!zipParameters.isSourceExternalStream() && file == null){ throw new ZipException("input file is null"); }

		if(!zipParameters.isSourceExternalStream() && !Zip4jUtil.checkFileExists(file)){ throw new ZipException("input file does not exist"); }
		...
		try{
			...
			if(!zipParameters.isSourceExternalStream()){
				...
				if(!Zip4jUtil.isStringNotNullAndNotEmpty(this.zipParameters.getFileNameInZip())){ throw new ZipException("file name is empty for external stream"); }
	...
	public void write(byte[] b, int off, int len) throws IOException{
		...
		if(zipParameters.isEncryptFiles() && zipParameters.getEncryptionMethod() == Zip4jConstants.ENC_METHOD_AES){
	...
	public void closeEntry() throws IOException, ZipException{
		...
		if(this.zipParameters.isEncryptFiles() && this.zipParameters.getEncryptionMethod() == Zip4jConstants.ENC_METHOD_AES){
	...
	private void createFileHeader() throws ZipException{
		...
		if(zipParameters.isSourceExternalStream()){
			...
			if(!Zip4jUtil.isStringNotNullAndNotEmpty(zipParameters.getFileNameInZip())){ throw new ZipException("fileNameInZip is null or empty"); }
			...
			fileName = Zip4jUtil.getRelativeFileName(sourceFile.getAbsolutePath(), zipParameters.getRootFolderInZip(), zipParameters.getDefaultFolderPath());

		}

		if(!Zip4jUtil.isStringNotNullAndNotEmpty(fileName)){ throw new ZipException("fileName is null or empty. unable to create file header"); }

		fileHeader.setFileName(fileName);

		if(Zip4jUtil.isStringNotNullAndNotEmpty(zipModel.getFileNameCharset())){
			fileHeader.setFileNameLength(Zip4jUtil.getEncodedStringLength(fileName, zipModel.getFileNameCharset()));
		}else{
			...
			if(!zipParameters.isSourceExternalStream()){
				...
				if(zipParameters.getCompressionMethod() == Zip4jConstants.COMP_STORE){
					if(zipParameters.getEncryptionMethod() == Zip4jConstants.ENC_METHOD_STANDARD){
						...
						fileHeader.setCompressedSize(fileSize + saltLength + InternalZipConstants.AES_AUTH_LENGTH + 2); // 2
																														// is
																														// password
																														// verifier
					}else{
		...
		if(zipParameters.isEncryptFiles() && zipParameters.getEncryptionMethod() == Zip4jConstants.ENC_METHOD_STANDARD){
		...
		if(isFileNameCharsetSet && zipModel.getFileNameCharset().equalsIgnoreCase(InternalZipConstants.CHARSET_UTF8) || !isFileNameCharsetSet && Zip4jUtil.detectCharSet(fileHeader.getFileName()).equals(InternalZipConstants.CHARSET_UTF8)){
			shortByte[1] = 8;
		}else{
			shortByte[1] = 0;
		}
	...
	private void createLocalFileHeader() throws ZipException{
		if(fileHeader == null){ throw new ZipException("file header is null, cannot create local file header"); }
	...
	private int getFileAttributes(File file) throws ZipException{
		if(file == null){ throw new ZipException("input file is null, cannot get file attributes"); }
	...
	private AESExtraDataRecord generateAESExtraDataRecord(ZipParameters parameters) throws ZipException{
		...
		if(parameters == null){ throw new ZipException("zip parameters are null, cannot generate AES Extra Data record"); }
		...
		// Always set the version number to 2 as we do not store CRC for any AES
		// encrypted files
		// only MAC is stored and as per the specification, if version number is
		// 2, then MAC is read
		// and CRC is ignored
