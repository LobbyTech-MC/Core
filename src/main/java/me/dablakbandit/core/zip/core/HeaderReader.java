package me.dablakbandit.core.zip.core;
...
import me.dablakbandit.core.zip.exception.ZipExceptionConstants;
import me.dablakbandit.core.zip.model.AESExtraDataRecord;
import me.dablakbandit.core.zip.model.CentralDirectory;
import me.dablakbandit.core.zip.model.DigitalSignature;
import me.dablakbandit.core.zip.model.EndCentralDirRecord;
import me.dablakbandit.core.zip.model.ExtraDataRecord;
import me.dablakbandit.core.zip.model.FileHeader;
import me.dablakbandit.core.zip.model.LocalFileHeader;
import me.dablakbandit.core.zip.model.Zip64EndCentralDirLocator;
import me.dablakbandit.core.zip.model.Zip64EndCentralDirRecord;
import me.dablakbandit.core.zip.model.Zip64ExtendedInfo;
import me.dablakbandit.core.zip.model.ZipModel;
import me.dablakbandit.core.zip.util.InternalZipConstants;
import me.dablakbandit.core.zip.util.Raw;
import me.dablakbandit.core.zip.util.Zip4jConstants;
...
public class HeaderReader{
	...
	private EndCentralDirRecord readEndOfCentralDirectoryRecord() throws ZipException{
		...
		if(zip4jRaf == null){ throw new ZipException("random access file was null", ZipExceptionConstants.randomAccessFileNull); }
		...
		try{
			...
			if(Raw.readIntLittleEndian(ebs, 0) != InternalZipConstants.ENDSIG){ throw new ZipException("zip headers not found. probably not a zip file"); }
	...
	private CentralDirectory readCentralDirectory() throws ZipException{
		...
		if(zipModel.getEndCentralDirRecord() == null){ throw new ZipException("EndCentralRecord was null, maybe a corrupt zip file"); }
		...
		try{
			...
			for(int i = 0; i < centralDirEntryCount; i++){
				...
				if(signature != InternalZipConstants.CENSIG){ throw new ZipException("Expected central directory entry not found (#" + (i + 1) + ")"); }
				...
				if(fileNameLength > 0){
					...
					if(fileName == null){ throw new ZipException("fileName is null when reading central directory"); }
			...
			readIntoBuff(zip4jRaf, intBuff);
			int signature = Raw.readIntLittleEndian(intBuff, 0);
			if(signature != InternalZipConstants.DIGSIG){ return centralDirectory; }
	...
	private void readAndSaveExtraDataRecord(FileHeader fileHeader) throws ZipException{
		...
		if(zip4jRaf == null){ throw new ZipException("invalid file handler when trying to read extra data record"); }

		if(fileHeader == null){ throw new ZipException("file header is null"); }
	...
	private void readAndSaveExtraDataRecord(LocalFileHeader localFileHeader) throws ZipException{
		...
		if(localFileHeader == null){ throw new ZipException("file header is null"); }
	...
	private Zip64EndCentralDirLocator readZip64EndCentralDirLocator() throws ZipException{
		...
		if(zip4jRaf == null){ throw new ZipException("invalid file handler when trying to read Zip64EndCentralDirLocator"); }
	...
	private Zip64EndCentralDirRecord readZip64EndCentralDirRec() throws ZipException{
		...
		if(zipModel.getZip64EndCentralDirLocator() == null){ throw new ZipException("invalid zip64 end of central directory locator"); }

		long offSetStartOfZip64CentralDir = zipModel.getZip64EndCentralDirLocator().getOffsetZip64EndOfCentralDirRec();

		if(offSetStartOfZip64CentralDir < 0){ throw new ZipException("invalid offset for start of end of central directory record"); }
		...
		try{
			...
			readIntoBuff(zip4jRaf, intBuff);
			int signature = Raw.readIntLittleEndian(intBuff, 0);
			if(signature != InternalZipConstants.ZIP64ENDCENDIRREC){ throw new ZipException("invalid signature for zip64 end of central directory record"); }
	...
	private void readAndSaveZip64ExtendedInfo(FileHeader fileHeader) throws ZipException{
		if(fileHeader == null){ throw new ZipException("file header is null in reading Zip64 Extended Info"); }

		if(fileHeader.getExtraDataRecords() == null || fileHeader.getExtraDataRecords().size() <= 0){ return; }
	...
	private void readAndSaveZip64ExtendedInfo(LocalFileHeader localFileHeader) throws ZipException{
		if(localFileHeader == null){ throw new ZipException("file header is null in reading Zip64 Extended Info"); }

		if(localFileHeader.getExtraDataRecords() == null || localFileHeader.getExtraDataRecords().size() <= 0){ return; }
	...
	private Zip64ExtendedInfo readZip64ExtendedInfo(ArrayList extraDataRecords, long unCompressedSize, long compressedSize, long offsetLocalHeader, int diskNumberStart) throws ZipException{
		...
		for(int i = 0; i < extraDataRecords.size(); i++){
			...
			if(extraDataRecord.getHeader() == 0x0001){
				...
				if(valueAdded){ return zip64ExtendedInfo; }
	...
	public LocalFileHeader readLocalFileHeader(FileHeader fileHeader) throws ZipException{
		if(fileHeader == null || zip4jRaf == null){ throw new ZipException("invalid read parameters for local header"); }
		...
		if(locHdrOffset < 0){ throw new ZipException("invalid local header offset"); }
		...
		try{
			...
			if(sig != InternalZipConstants.LOCSIG){ throw new ZipException("invalid local header signature for file: " + fileHeader.getFileName()); }
			...
			localFileHeader.setCrcBuff(intBuff.clone());
			length += 4;
			...
			if(fileNameLength > 0){
				...
				if(fileName == null){ throw new ZipException("file name is null, cannot assign file name to local file header"); }
	...
	private AESExtraDataRecord readAESExtraDataRecord(ArrayList extraDataRecords) throws ZipException{
		...
		if(extraDataRecords == null){ return null; }
		...
		for(int i = 0; i < extraDataRecords.size(); i++){
			...
			if(extraDataRecord.getHeader() == InternalZipConstants.AESSIG){
				...
				if(extraDataRecord.getData() == null){ throw new ZipException("corrput AES extra data records"); }
	...
	private byte[] getLongByteFromIntByte(byte[] intByte) throws ZipException{
		if(intByte == null){ throw new ZipException("input parameter is null, cannot expand to 8 bytes"); }

		if(intByte.length != 4){ throw new ZipException("invalid byte length, cannot expand to 8 bytes"); }
